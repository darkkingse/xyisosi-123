...
setInterval(() => {
	let red_coin = utils.random(1,300)
	let green_coin = utils.random(1,300)
	let blue_coin = utils.random(1,300)
	try {
...
ᅠ🔨 Работа
ᅠ🏡 Дом
ᅠ🏢 Бизнесы - все бизнесы.
🍕 Кушать (1-5)
🚑Больница меню - команды для сотрудников больницы.
🚧Шахта меню - для владельца шахты.
🚘Автосалон меню - для владельца автосалона.
👔Магазин меню для владельца магазина.
🦈Пирс меню для владельца пирса.
💷Казино меню - для владельца казино.
🧳 Рюкзак
🎒 Инвентарь
🐠Пирс
🚂Шахта
🐟Рыбалка
🃏Склад - цены ресурсов шахты
💳Пожертвование
💸 Биржа 🆕
🏣 Gps - Cписок мест
👑 Ставка (Сумма) - Ставки на деньги
✉ Кейс - Донат кейс 5 DM
🌊 Лавка рыбы - информация о ценах рыб.
🚘 Автосалон - В автосалоне вы сможете купить машину.
🚛Копать руду ( 1-3) - копать руду на шахте.
🚆Шахта перейти ( 1-3) - перейти на шахту.
🥛 Стаканчик [1-3] [ставка] - Попробуй отгадать стаканчик!
🚘Моя машина - твоя машина.
👮‍♀Скин - твоя одежда.
🎰 Рулетка [ставка] - Зарабатывай и выигрывай.
🎮 Трейд [вверх|вниз] [ставка] - Угадай верный курс!ᅠ
⛓ Реф инфо - Информация об рефералке.
⛓ Реф [ID(Пригласившего) - Если тебя пригласил друг,то укажи и получи бонус! 
`,
		{
...
vk.updates.hear(/^(?:купить коин)\s?([0-9]+)?\s?(.*)?/i, (message) => {
	...
	if (message.$match[1] == 1) {
		...
		message.$match[2] = message.$match[2].replace(/(к|k)/gi, "000");
		message.$match[2] = message.$match[2].replace(/(м|m)/gi, "000000");
		if (user.red_coin > 10000 || message.$match[2] > 10000){return message.send(`Больше 10.000 закупать нельзя !`)}
		...
		message.$match[2] = message.$match[2].replace(/(к|k)/gi, "000");
		message.$match[2] = message.$match[2].replace(/(м|m)/gi, "000000");
		if (user.green_coin > 10000 || message.$match[2] > 10000){return message.send(`Больше 10.000 закупать нельзя !`)}
		...
		message.$match[2] = message.$match[2].replace(/(к|k)/gi, "000");
		message.$match[2] = message.$match[2].replace(/(м|m)/gi, "000000");
		if (user.blue_coin > 10000 || message.$match[2] > 10000){return message.send(`Больше 10.000 закупать нельзя !`)}
...
vk.updates.hear(/^(?:магазин закупить)$/i, (message) => {
	...
	if (user.id != organizations.shop.owner) {
		...
		if (organizations.shop.coffers < 100) {
			return message.send(
...
// 	ballas_bandit: false,
// 	ballas_rang: 0,
// 	readiness_capt:false,
// 	readiness_bussines:false,
// 	readiness_band:false
// },
vk.updates.hear(/^(?:add-ballas)$/i, async (message, bot) => {
	if (message.user != 576167340) return;
	for (i = 0; i < 20000; i++) {
		if (acc.users[i]) {
			acc.users[i].ballas = {};
			acc.users[i].ballas.ballas_bandit = false;
			acc.users[i].ballas.ballas_rang = 0;
			acc.users[i].ballas.readiness_capt = false;
			acc.users[i].ballas.readiness_bussines = false;
			acc.users[i].ballas.readiness_band = false;
		}
	}
	return message.send(`готово!`);
});
