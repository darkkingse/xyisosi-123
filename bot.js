...
vk.updates.hear(/^(?:коткрыть 1)/i, (message) => {
	let user = acc.users[u_id(message.user)];
	if (user.balance < 250000) return message.send(`‼ У вас недостаточно денег на игровом счету.`)
	containers.russia.cars.count -= 1;
	let car = utils.random(21,22)
	user.car = car;
	return message.send(
		`Вы открыли контейнер и там была машина: ${cars[car].name}🚘`,
	)
})
...
vk.updates.hear(/^(?:авто продать|🚫 Авто продать)/i, (message) => {
	let user = acc.users[u_id(message.user)];
	if (!user.car_case) return message.send(`❗ Вам не выпадала машина с кейса`)
	user.balance += cars[user.car_case].price * 0.7
	message.send(`✅ Вы успешно продали Автомобиль за: ${cars[user.car_case].price * 0.7}$`)
	user.car_case = false;
})
vk.updates.hear(/^(?:авто забрать|✅ Авто забрать)/i, (message) => {
	...
	user.car == false ? user.car = Number(user.car_case) : !user.car2 ? user.car2 = Number(user.car_case) : !user.car3 ? user.car3 = Number(user.car_case) : ''
	
	message.send(`✅ Вы успешно забрали Автомобиль:${cars[user.car_case].name}`)
	user.car_case = false;
})
...
vk.updates.hear(/^(?:мерия|мэрия)$/i, (message) => {
	...
	return message.send(` Вы перешли в Мэрию 🏛 
		...
		➖➖➖➖➖➖
		1⃣.Получить паспорт
		2⃣.Выборы президента штата
	    3️⃣.Узнать владельца Lux-Bussines.  
...
vk.updates.hear(/^(?:выбрать 3)\s?([0-1]+)?/i, (message) => {
