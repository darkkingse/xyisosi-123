...
vk.updates.hear(/^(?:машина)\s?([0-9]+)?/i, (message) => {
	...
	user.car = Number(message.$match[1]);
...
vk.updates.hear(/^(?:ставка|🎲 Ставка)\s?(.*)?/i, (message) => {
	...
	if (args[1] <= user.balance && user.balance <= 50000000000) {
		...
		const multiply = utils.pick([
			0, 0, 2, 0, 50, 1, 0.75, 2, 1, 0, 5, 2, 3, 3, 3, 1, 2, 2, 1, 0.25, 0.25,
...
vk.updates.hear(/^(?:кейс открыть 4|📦 Кейс открыть 4)/i, (message) => {
	...
	user.car = car;
	return message.send(
		`Вы выиграли ${cars[car].name}🚘\n Что посмотреть свою машину, используйте "Моя машина" `,
		{
			keyboard: JSON.stringify({
				inline: true,
				buttons: [
...
vk.updates.hear(/^(?:подарок)/i, (message) => {
	...
	if (user.podarok == true)
		return message.send(
			`❗ Ошибка, Подарок можно получить только раз в 15 минут.`
		);
	user.podarok = true;
	setTimeout(() => {
		user.podarok = false;
...
vk.updates.hear(/^(?:донат)\s?([0-9]+)?/i, (message) => {
	return message.send(`
		...
		🔧Супер кирка ( х3 ресурсы на шахте) - 500 рублей.
		🔭Супер удочка ( х3 рыб на рыбалке) - 350 рублей.

		⛔ По поводу покупки писать Руководителю (ссылку). 
	`);
});
