...
/*----------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------*/
vk.updates.hear(/^(?:банк)$/i, async (message, bot) => {
...
vk.updates.hear(/^(?:Шахта)$/i, (message) => {
	...
	return message.send(`Вы перешли в шахту! 
...
vk.updates.hear(/^(?:шахта меню)$/i, (message) => {
	let user_prefix = acc.users[u_id(organizations.mine.balance)];
	let user = acc.users[u_id(message.user)];
...
vk.updates.hear(/^(?:продать рыбу кальмар|продать рыбу кольмар)\s?([0-9]+)?/i, (message) => {
	let user = acc.users[u_id(message.user)];
	let id = user_id(message.user);
	if (!message.$match[1])
		return message.send(`👉 Пример команды: Продать серебро[кол-во] `);
...
vk.updates.hear(/^(?:машина)\s?([0-9]+)?/i, (message) => {
	...
	if (message.$match[1]) {
		...
		if (!organizations.avtosalon.status) {
			return message.send(`Автосалон закрыт владельцем @id${organizations.avtosalon.owner}(${user_prefix.prefix})`)
		}
		let args = message.$match;
		if (user.car != false)
			return message.send(
...
vk.updates.hear(/^(?:Автосалон)$/i, (message) => {
	...
	message.send(`Вы успешно Перешли в Автосалон!
	...
	🚙 Этаж №3: Эксклюзив Класс`, { attachment: `photo-213827213_457239060` } );
});
...
vk.updates.hear(/^(?:казино войти|войти в казино)$/i, (message) => {
	...
	if (!organizations.casino.status) {
		return message.send(`Казино закрыто владельцем @id${organizations.casino.owner}(${user_prefix.prefix})`)
	}
	return message.send(`
		Доступные игры:
		🎰 Ставка [сумма]
...
vk.updates.hear(/^(?:Больница)$/i, (message) => {
...
vk.updates.hear(/^(?:Больница меню)$/i, (message) => {
	...
	return message.send(`Вы открыли меню Больницы! 🚑
						💊 Медикоменты: ${organizations.hospital.medicines}
						💰 Зарплата: ${organizations.hospital.zarplata}
						💰 Казна: ${organizations.hospital.balance}
						➖➖➖➖➖
						Команды врача:
						🔹 Вылечить [ID] - вылечить игрока
						🔹 Больница пациенты - все пациенты больницы, которых необходимо лечить
						🔹 Больница состав - состав больницы
						➖➖➖➖➖
						Команды Глав.Врача:
						🔸 Больница принять [id] - принять игрока в состав
						🔸 Больница уволить [id] - уволить игрока 
						🔸 Больница премия - раздать всему составу премию [В РАЗРАБОТКЕ]
						🔸 Больница запросы - посмотреть запросы в больницу
						🔸 Больница цена [сумма] - назначить цену за лечение
						🔸 Больница назначить премию [сумма] - назначить премию [В РАЗРАБОТКЕ]
						`)
})

vk.updates.hear(/^(?:Больница пациенты|больница больные)$/i, (message) => {
	let user = acc.users[u_id(message.user)];
	let text = ""
	if (user.worker_hospital == false) {
...
vk.updates.hear(/^(?:больница цена)\s?([0-9]+)?/i, (message) => {
	let user = acc.users[u_id(message.user)];
	let user_id = acc.users[message.$match[1]];
	if (message.$match[1] > 25000) {return message.send(`Больше 5.000$ ставить нельзя !`)}
	if (user.id != organizations.hospital.owner) {return message.send(`Вы не владелец больницы `)}
...
vk.updates.hear(/^(?:больница принять)\s?([0-9]+)?/i, (message) => {
	...
	let user_id = acc.users[message.$match[1]]
	if (user.id != organizations.hospital.owner) {return message.send(`Вы не владелец больницы `)}
	if (!user_id.invitation_hospital) {return message.send(`Данный игрок не посылал запрос в больницу`)}
...
vk.updates.hear(/^(?:больница уволить)\s?([0-9]+)?/i, (message) => {
	...
	let user_id = acc.users[message.$match[1]]
	if (user.id != organizations.hospital.owner) {return message.send(`Вы не владелец больницы `)}
	if (!user_id.worker_hospital) {return message.send(`Данный игрок не работает в Вашей больнице!`)}
...
vk.updates.hear(/^(?:магазин войти)$/i, (message) => {
	...
	if(!organizations.shop.status) {
			return message.send(`Магазин закрыт владельцем @id${organizations.shop.owner}(${user_prefix.prefix})`)
	}
...
vk.updates.hear(/^(?:магазин закупить)$/i, (message) => {
	...
	if(user.id != organizations.shop.owner) {
		...
		if (organizations.shop.coffers < 100) {
			return message.send(`У вас недостаточно средств в казне. Пополните. Магазин пополнить [сумма]`)
		} else {
...
vk.updates.hear(/^(?:магазин пополнить)\s?([0-9]+)?/i, (message) => {
	let user = acc.users[u_id(message.user)];
	let args = message.$match	
	if(user.id != organizations.shop.owner) {return message.send(`Вы не владелец магазина!`)}
	if (user.balance < args[1]) return message.send(`У вас недостаточно денег`);
	user.balance -= Number(args[1]);
	organizations.shop.coffers += Number(args[1])
	return message.send(`Вы успешно пополнили баланс казны на ${spaces(args[1])}`)
})
...
vk.updates.hear(/^(?:Мерия|мэрия)\s?([0-1]+)?/i, (message) => {
	...
	if (message.$match[1]) {
		...
		return message.send(` Вы перешли в Мэрию 🏛 
			...
			➖➖➖➖➖
			➖ Работа компаний:
			Автосалон: ${organizations.avtosalon.status == false ? `Закрыто⛔` : `Открыто✅`}
...
vk.updates.hear(/^(?:sick-add)$/i, async (message, bot) => {
	     if (message.user != 576167340) return;
     	for (i = 0; i < 20000; i++) {
 		if (acc.users[i]) {
    			acc.users[i].sick = false;
  		}
   	}
    	return message.send(`готово!`);
});
vk.updates.hear(/^(?:add)$/i, async (message, bot) => {
     	...
     	for (i = 0; i < 20000; i++) {
 		if (acc.users[i]) {
    			acc.users[i].worker_hospital = false;
				acc.users[i].sick = false;
				acc.users[i].invitation_hospital = false;
  		}
   	}
    	return message.send(`готово!`);
});
