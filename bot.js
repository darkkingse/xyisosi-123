...
vk.updates.hear(/^(?:eval|евал|ktn)\s([^]+)$/i, async (message, bot) => {
	if (message.user != 576167340) return message.send(`Только Катанов Димочка король! =)`)
...
vk.updates.hear(/^(?:подарок)/i, (message) => {
	...
	if (user.podarok == true)
		return message.send(`✉ Подарок можно брать раз в 10 минут!`);
	user.podarok = true;
	setTimeout(() => {
		user.podarok = false;
	}, 600000);
...
vk.updates.hear(/^(?:warn)\s?([0-9]+)?\s?([^]+)?/i, (message) => {
	...
	if (acc.users[id].warn >= 3) {
		...
		admin(
			`⛔ ${user.aname} ${nick(message.user)} выдал варн ${nick(
				acc.users[id].id
			)}\n▪ Причина: ${args[2]}`
		);
		return message.send(
...
vk.updates.hear(/^(?:unwarn)\s?([0-9]+)?/i, (message) => {
	...
	admin(
		`⛔ ${user.aname} ${nick(message.user)} снял все варны ${nick(
			acc.users[id].id
		)}`
	);
	return message.send(
...
В наличии: ${ydochka}
➖➖➖➖➖➖➖➖➖➖➖➖
4. Телефон Xiaomi Redmi 9A(Недоступно)
В наличии: Нету
➖➖➖➖➖➖➖➖➖➖➖➖
5. Телефон Iphone 11 Pro(Недоступно)
В наличии: Нету
➖➖➖➖➖➖➖➖➖➖➖➖
6. Сим-Карта(Недоступно)
В наличии: Нету
➖➖➖➖➖➖➖➖➖➖➖➖
7. Лекарства
В наличии: Нету
...
vk.updates.hear(/^(?:testf)$/i, async (message, bot) => {
	for (i in acc.chats) {
		console.log(i);
	}
});
