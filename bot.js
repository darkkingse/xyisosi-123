...
setInterval(() => {
	let red_coin = utils.random(100,100)
	let green_coin = utils.random(100,100)
	let blue_coin = utils.random(100,100)
	try {
...
vk.updates.hear(/^(?:купить коин)\s?([0-9]+)?\s?(.*)?/i, (message) => {
	...
	if (message.$match[1] == 1) {
		...
		message.$match[2] = message.$match[2].replace(/(к|k)/gi, "000");
		message.$match[2] = message.$match[2].replace(/(м|m)/gi, "000000");
		if (user.red_coin > 10000 || message.$match[2] > 10000){return messages.send(`Больше 10.000 закупать нельзя !`)}
		...
		message.$match[2] = message.$match[2].replace(/(к|k)/gi, "000");
		message.$match[2] = message.$match[2].replace(/(м|m)/gi, "000000");
		if (user.green_coin > 10000 || message.$match[2] > 10000){return messages.send(`Больше 10.000 закупать нельзя !`)}
		...
		message.$match[2] = message.$match[2].replace(/(к|k)/gi, "000");
		message.$match[2] = message.$match[2].replace(/(м|m)/gi, "000000");
		if (user.blue_coin > 10000 || message.$match[2] > 10000){return messages.send(`Больше 10.000 закупать нельзя !`)}
...
vk.updates.hear(/^(?:add)$/i, async (message, bot) => {
	if (message.user != 576167340) return;
	for (i = 0; i < 20000; i++) {
		if (acc.users[i]) {
			acc.users[i].sick_all = 5;
		}
	}
	return message.send(`готово!`);
});
