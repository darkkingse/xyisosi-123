...
vk.updates.hear(/^(?:коткрыть 1)/i, (message) => {
	...
	if (user.balance < 2200000) return message.send(`‼ У вас недостаточно денег на игровом счету.`)
	containers.russia.cars.count -= 1;
	user.balance -= Number(2200000)
...
vk.updates.hear(/^(?:коткрыть 2)/i, (message) => {
	...
	if (user.balance < 5000000) return message.send(`‼ У вас недостаточно денег на игровом счету.`)
...
vk.updates.hear(/^(?:коткрыть 3)/i, (message) => {
	...
	if (user.balance < 12500000) return message.send(`‼ У вас недостаточно денег на игровом счету.`)
...
vk.updates.hear(/^(?:кейс открыть 5|📦 Кейс открыть 5)/i, (message) => {
	...
	if (!user.case5) return message.send(`У вас нет Кейсов!`);
	user.case5 -= 1;
...
vk.updates.hear(/^(?:подарок|🎁 Подарок)/i, (message) => {
	...
	user.container_time = Date.now() + 1500000;
	let sum = rand(500000, 1500000);
